...
import { NgOption, KeyCode, NgSelectConfig } from '../../models/ng-select.types';
...
export class NgSelectComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    selectTag() {
        ...
        if (this.addTag instanceof Function) {
            tag = (this.filterValue !== null) && this.addTag(this.filterValue);
        } else {
            tag = (this.filterValue !== null) && { [this.bindLabel]: this.filterValue };
        }

        if (tag instanceof Promise) {
            tag.then(item => (this.filterValue !== null) && this.select(this.itemsList.addItem(item)))
                .catch(() => { });
        } else if (tag) {
            // tslint:disable-next-line:no-unused-expression
            (this.filterValue !== null) && this.select(this.itemsList.addItem(tag));
        }
    }
    ...
    private _validateWriteValue(value: any) {
        ...
        if (this.multiple) {
            ...
            if (!Array.isArray(value) && this._isDefined(value)) {
    ...
    private _selectWriteValue(ngModel: any | any[]) {
        ...
        const select = (val: any) => {
            ...
            const item = this.itemsList.findItem(val.text);
        ...
        if (this.multiple) {
            const c = Object.values(ngModel);
            (<any[]>c).forEach((item, index) => {
                select(item);
            });
        } else {
            select(ngModel);
